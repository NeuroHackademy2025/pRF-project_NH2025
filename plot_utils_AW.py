...
from math_utils import weighted_regression

def prf_ecc_pcm_plot_matplotlib(df_ecc_pcm, data_pcm_raw, fig_width, fig_height, rois, roi_colors, plot_groups, max_ecc):
    """
    Make scatter plot for relationship between eccentricity and pCM (Matplotlib version)
    """
    # Create figure and subplots
    cols = len(plot_groups)
    fig, axes = plt.subplots(1, cols, figsize=(fig_width / 100, fig_height / 100), sharey=True)
    if cols == 1:  # Ensure axes is iterable
        axes = [axes]
    ...
    for l, line_label in enumerate(plot_groups):
        ...
        for j, roi in enumerate(line_label):
            ...
            # Regression line
            line_x = np.array(data_pcm_raw_roi['prf_ecc'])
            line_y = 1 / (slope * line_x + intercept)

            # Horton prediction
            horton_line = 17.3 / (line_x + 0.75)
            ...
            # Scatter: raw data
            ax.scatter(data_pcm_raw_roi['prf_ecc'], data_pcm_raw_roi['pRF_CM'], 
                       color=roi_color, s=20, alpha=0.4, label=None)
            # Regression line
            ax.plot(line_x, line_y, color=roi_color, linewidth=2)
            # Horton line
            ax.plot(line_x, horton_line, color='black', linewidth=2, linestyle='--')

            # Annotation for R²
            ax.text(5, 20 - j*1.5, f'{roi}, $R^2$={r2:.2f}', color=roi_color, fontsize=12)

        # Axes labels and limits
        ax.set_xlim([0, max_ecc])
        ax.set_ylim([0, 20])
        ax.set_xlabel('pRF eccentricity (dva)')
        if l == 0:
            ax.set_ylabel('pRF cortical magn. (mm/dva)')
